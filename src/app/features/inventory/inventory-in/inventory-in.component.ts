...
import { InventoryInRequest } from '../../../core/models/inventory.model';
...
@Component({
  ...
  styleUrl: './inventory-in.component.scss',
})
export class InventoryInComponent implements OnInit {
  ...
  readonly form = this.fb.group({
    ...
    unitPrice: [null as number | null, [Validators.required, CustomValidators.nonNegative()]],
  ...
  submit(): void {
    ...
    const value = this.form.getRawValue() as InventoryInRequest;
    ...
    this.inventoryService
      ...
      .subscribe({
        ...
        error: (err: HttpErrorResponse) => {
          ...
          const msg = tryGetMessage(err) ?? 'Validation failed.';
  ...
  apiError(controlName: 'productId' | 'warehouseId' | 'quantity' | 'unitPrice' | 'notes'): string | null {
  ...
  private clearApiErrors(): void {
    for (const controlName of ['productId', 'warehouseId', 'quantity', 'unitPrice', 'notes'] as const) {
  ...
  private applyFieldErrors(errors: FieldErrors): void {
    const mapKeyToControl: Record<string, 'productId' | 'warehouseId' | 'quantity' | 'unitPrice' | 'notes'> = {
      productid: 'productId',
